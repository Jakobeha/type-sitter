...
pub(super) enum SExpNodeType {
    ...
    Union {
        types: VecDeque<NodeType>,
        are_all_variants_named: bool,
    },
    Untyped {
        is_named: bool,
    },
...
impl SExpNodeType {
    pub(super) fn union_with(self, other: Self) -> Self {
        match (self, other) {
            (this, Self::Untyped { is_named }) => Self::Untyped {
                is_named: this.is_named() && is_named,
            },
            (
                Self::Untyped {
                    is_named: self_is_named,
                },
                other,
            ) => Self::Untyped {
                is_named: self_is_named && other.is_named(),
            },
            ...
            (
                Self::Union {
                    types: mut self_types,
                    are_all_variants_named: self_are_all_variants_named,
                },
                Self::Union {
                    mut types,
                    are_all_variants_named,
    ...
    pub(super) fn rust_type_path(&self, nodes: &Path, capture_variant_name: &str) -> Cow<'_, str> {
        match self {
            ...
            Self::Single { r#type } => Cow::Owned(format!(
                "{}::{}",
                nodes.to_token_stream().to_string().replace(" ", ""),
                r#type.rust_type_path()
            )),
    ...
    pub(super) fn node_type(&self, is_head: bool, all_types: &NodeTypeMap) -> SExpNodeType {
        match self {
            ...
            SExp::Group {
                ...
                GroupType::Bracket => items
                    .iter()
                    .map(|item| item.node_type(is_head, all_types))
                    .collect(),
            },
        }
    }
