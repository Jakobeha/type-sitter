...
pub fn process(item: &InOutPair, args: &Args, tree_sitter: &syn::Path) -> errors::Result<()> {
    // Get input type
    ...
    do_process(&item.input, &output_path, input_type, language_dir.as_deref(), tree_sitter)
}
...
fn do_process(
    ...
    match input_type {
        ...
        InputType::Query => {
            ...
            write(&output_path, type_sitter_gen::generate_queries(input_path, language_dir, &nodes(), &tree_sitter)?)?;
        }
